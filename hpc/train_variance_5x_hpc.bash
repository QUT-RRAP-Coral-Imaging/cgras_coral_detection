...
# Use the cgras configuration (amag140) as the base dataset config
BASE_CONFIG="${BASE_CONFIG:-${REPO_ROOT}/segmenter/config/cgras/amag140.yaml}"
# Use the cgras training template which contains common training defaults
TRAIN_TEMPLATE="${TRAIN_TEMPLATE:-${REPO_ROOT}/segmenter/config/cgras/amag140.yaml}"
RUNS="${RUNS:-5}"
# Write outputs into a cgras-specific analysis directory
...
# Try known user install locations first (legacy paths)
if [ -f /home/wardlewo/miniforge3/bin/activate ]; then
  ...
  # Fallback: if conda is on PATH, use the shell hook to activate the environment
  if command -v conda >/dev/null 2>&1; then
    # Initialize conda for non-interactive shells and activate the named env
...
echo "Repeats: ${RUNS}" | tee -a "$LOG_FILE"
...
for i in $(seq 1 "$RUNS"); do
  RUN_TAG="${RUN_GROUP}_run${i}"
  TMP_CONFIG="${OUTPUT_DIR}/${RUN_TAG}.yaml"
  RUN_SEED=$((BASE_SEED + i - 1))

  echo "==================================================" | tee -a "$LOG_FILE"
  echo "[Run ${i}/${RUNS}] Preparing config: ${TMP_CONFIG} (seed=${RUN_SEED})" | tee -a "$LOG_FILE"
...
base_yaml_path = str(base_cfg)
if any(key in base_data for key in ("model_path", "task", "epochs", "batch_size", "yaml_path")):
    cfg.update(base_data)
else:
    cfg["yaml_path"] = base_yaml_path
...
PY
  ...
  echo "[Run ${i}/${RUNS}] Starting training..." | tee -a "$LOG_FILE"
...
PY
  echo "[Run ${i}/${RUNS}] Completed." | tee -a "$LOG_FILE"
done

echo "==================================================" | tee -a "$LOG_FILE"
echo "All ${RUNS} non-deterministic runs completed." | tee -a "$LOG_FILE"
echo "Summary log: ${LOG_FILE}" | tee -a "$LOG_FILE"

conda deactivate
echo "variance job done"